...
from cruddy.books_model import books
import random
...
def users_ilike(term):
    ...
    print(term)
    table = books.query.order_by(books.name).filter((books.name.ilike(term)) | (books.email.ilike(term)))
    for p in table:
        print(p.read())
